...
class Boundary {
public:
  static void apply_boundaries(Fields &fields, const Domain &domain, double Th,
                               double Tc);
  ...
  static void apply_pressure(Matrix &p, const Domain &domain);// {
    //int imaxb = domain.imax + 2;
    //int jmaxb = domain.jmax + 2;
    //for (int j = 0; j < jmaxb; j++) {
    //  // Left BC
...
extern void Boundary_kernel(Fields& fields, const Domain& domain, double Th, double Tc);
